...
1. **G1（已验 2026-08-03）**：`rollysys/use_cninfo` = **MIT ✅（fetch + PyMuPDF markdown 轮子可用）**；`alicexl/a-share-financials` **无 LICENSE（main + master 均 404）→ G1 fail，不可采用**。→ PDF→结构化数值解析须 **Aionis 自建**（复用 PyMuPDF MIT 工具 + 项目特定解析逻辑，属"适配"非"造轮子"）或 intake 时另找 permissive 轮子。cninfo 数据再分发 ToS 仍待核（可能 G6 exploratory-only/禁再分发，类比 Reddit/PRAW）。
