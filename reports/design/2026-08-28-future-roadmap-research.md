...
### P0 —— 业主门/时间敏感
1. **E3 影子完整跑(下一窗口=2026-09-30 月末交易日,北京 10-01 04:00 后)**:**八月首影失守如实记录**(08-31 深夜宿主机休眠,自动化过期未执行)。轮 54 已武装:①触发器新增 `--run-date` CLI(UTC+8 收盘后执行日历已翻日,today 推导会瞄错月份——必须显式钉交易日);②volume 缓存新鲜度守卫(<4 日自动重抓);③研究价格面板已含 08-31(3,938×585;冻结面"ticker-complete 即终态"契约经测试拦截后回退并留注)。冒烟验证:`--run-date 2026-09-30` 正确识别月末→fail-closed 于 `predict_session_not_in_panel`(panel_max=2026-07-31)。**runbook(10-01 04:00 后按序)**:①`uv run python scripts/track_b_fetch_volume.py` ②`uv run python scripts/track_b_materialize_panel.py` ③`PHASE_E3_NO_LEDGER=1 uv run python scripts/e3_forward_trigger.py --run-date 2026-09-30`。**结构性发现待裁决(研究线)**:组装面板网格边缘随 fund/mem 节奏推进(现 07-31);membership 源为月度快照(max=2026-04-01),若边缘推进不及 09-30,readiness 将结构性失败——需裁决替代 PIT 成员源/容差策略/或接受当日如实失败顺延。**2026-09-01 门控硬化追加(如实)**:AUD-06 接线修复后,readiness 现在检查真实事件帧——`_events_df` 的 text 按构造为空串(主文档文本抓取切片未实现),故**面板追平 09-30 后 readiness 将额外 fail-closed 于 `llm_event_text_empty`**,直到该切片落地(arm_e13 的 LLM 边缘被静默跳过本就是 AUD-06 要防的缺陷——这是修复后的正确行为,不是回归);同批硬化:`scripts/forward_commit.py` shim 不再是无门旁路(与触发器同加载冻结契约+enforce)、`provider_cutoff_faked` 检测激活(cutoff 月=predict 月即拒绝)。**2026-09-02 轮 56 更新(两项已销)**:①**文本切片已落地**——`fetch_edgar_primary_text`(正典 URL+声明式 UA[浏览器 UA 被 www.sec.gov 403,声明式 200——实测]+≥2s 策略+幂等缓存+失败不缓存),AAPL/MSFT 真实 8-K 原文探针在案;`llm_event_text_empty` 门现可真实通过。②**membership 结构性阻塞已解除**——`universe_ext` 续造器(子代理实现+主线修复复合表头/月末语义/改名 no-op 后)以**封闭偏差清单**对账门通过 124 重叠月(281 项偏差全数逐项根因:上游现行 ticker 标注/承继行/末月文件滞后),历史 62,655 行逐位不变,新增 2026-05..09 五个月(503 只/月,月差分与真实事件一致),membership max=2026-09-01 距 09-30 ≈20 sessions < 22 契约。**runbook 追加步骤⓪:`uv run python scripts/extend_membership_wikipedia.py`(月末续造 membership,门失败即中止)**。
