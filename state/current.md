...
- **active (2026-08-09):** **Track A 切片 A1 已交付（待提交）** — 业主重提"实时更新+自校正参数往可观"；关键：此问题 2026-08-08 已实测 = Track B（扩窗周重训）**NULL（#54，IC_diff −0.0037，p=0.26）**。业主裁**路径甲（显示层自适应校准）**。本轮交付 A1：`score_calibration.calibrate_walk_forward()`（每个 realized 月 T 用严格 T 之前的 realized 对重拟合 Platt 显示映射 + 回测，per-month OOS-ECE 序列 + pooled 可靠性图；纯 display，0 ledger/frozen/E3/研究估计量）+ `export_calibration_reliability()`→`calibration_reliability.json` + 8 hermetic 测试。54 display 测试绿 + ruff clean + 真实数据跑（US pooled_ece 0.037 / 概率带紧绕 base rate = 诚实 null；ECE 随样本降 0.18→0.115）。**A2（漂移）已建 / A3（前瞻累计）= E3 owner-GO 门**。3 文件 +291/-2，0 ledger/frozen/data/web-artifact。待业主：审+授权 commit / 写 web JSON（前端 owner 协调）/ E3 GO。详见 `state/handoff.md` §2026-08-09(l)。
