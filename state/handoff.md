...
- **轮 91（2026-09-07）完整性审计+实弹验证：**
  ...
  - **轮 93（2026-09-08 晚，业主设计修正）**：①钩子窗口化——只在 18:00-23:59 存在（窗外零动作：不评估守卫/不读状态/不写日志=白天重启 ZCode 可观察地无事件；此前的行为是全天评估守卫后静默跳过=无副作用但状态消息可见，业主据此提出修正）；②同晚重启惯性——窗口内已完成后再触发只写一行 `hook skip:` 审计轨迹，绝不二次执行（证据：9-07/9-08 两晚日志各恰 1 次 evening lane start，once-per-day 从未破过）。测试 19 绿（窗外零文件断言+重启跳过轨迹断言）。statusMessage 改 "active 18:00-24:00 only"。
  - **轮 94（2026-09-08 晚）**：全牌审计（当前全绿/历史 10 红逐条归因：6 遗留+3 本通道事故均已闭环）；值班自动化第 4 步制度化升级为每晚双链全牌核验、红牌必报（CronUpdate 已生效）；教训入册——交付声明必须附带全牌状态，禁止只报末次绿。
